...
```mermaid
flowchart TD
    BI["BI & Analytics Layer<br>(Tableau, Power BI, Looker, Superset)"]
    QE["Query Engines<br>(Spark SQL, Trino, Presto, Dremio, StarRocks)"]
    OTF["Open Table Format<br>(Iceberg, Delta Lake, Hudi)"]
    STORE["Storage Layer<br>(Parquet, ORC, Avro on Object Storage)"]
    ...
    style BI fill:#e3f2fd,stroke:#1e88e5
    style QE fill:#fff9c4,stroke:#fbc02d
    style OTF fill:#e8f5e9,stroke:#4caf50
    style STORE fill:#f3e5f5,stroke:#ab47bc
    style OBJ fill:#ffe0b2,stroke:#f57c00
```
