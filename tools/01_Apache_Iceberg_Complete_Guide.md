...
```mermaid
flowchart TD
    QE["Query Engines<br>(Spark, Flink, Trino, Presto, Dremio, StarRocks)"]
    API["Iceberg API<br>(Java, Python, Rust, Go, C++)"]
    CAT["Catalog<br>(Hive Metastore, AWS Glue, REST, Nessie, JDBC, Hadoop, Custom)"]
    
    subgraph META [" "]
        direction TB
        M_TITLE["Iceberg Metadata"]
        style M_TITLE fill:none,stroke:none,font-weight:bold,color:#333
        
        PTR["Catalog Pointer<br>(points to current metadata)"]
        MFILE["Metadata File (JSON)<br>• Table schema<br>• Partition spec<br>• Current snapshot ID<br>• Snapshot history<br>• Properties"]
        SNAP["Snapshot<br>• Snapshot ID<br>• Timestamp<br>• Manifest list location<br>• Summary (added/deleted files)"]
        MLIST["Manifest List (Avro)<br>• List of manifest files<br>• Partition summaries per manifest"]
        MFILES["Manifest Files (Avro)<br>• Data file paths<br>• Partition values<br>• Column-level stats<br>• File size/count"]
        
        M_TITLE ~~~ PTR
        PTR --> MFILE --> SNAP --> MLIST --> MFILES
    end
    
    DATA["Data Files<br>(Parquet, ORC, Avro on S3, GCS, ADLS, HDFS)"]
    
    QE --> API --> CAT --> META --> DATA

    style META fill:#f3e5f5,stroke:#ab47bc
```
...
```mermaid
flowchart LR
    S1["Snap-1<br>[A]<br>(init)"]
    S2["Snap-2<br>[A+B]<br>(add B, append)"]
    S3["Snap-3<br>[A+C]<br>(delete B, add C)"]
    S4["Snap-4 (current)<br>[A+D]<br>(add D)"]
    
    S1 --> S2 --> S3 --> S4
    
    style S4 fill:#e3f2fd,stroke:#1e88e5,stroke-width:2px
```
...
flowchart LR
    subgraph PIPE [" "]
        direction LR
        TITLE["LinkedIn Analytics Pipeline"]
...
flowchart TD
    subgraph ECOM [" "]
        ...
        subgraph TABLES [" "]
            direction TB
            T_TITLE["Iceberg Tables"]
            style T_TITLE fill:none,stroke:none,font-weight:bold,color:#333
            TB["- page_views<br>- clicks<br>- purchases<br>- user_sessions"]
            T_TITLE ~~~ TB
        end
        
        TITLE ~~~ APP
        
        APP --> KAFKA --> FLINK --> TABLES
        
        TRINO["Real-time Dashboards<br>(Trino)"]
        SPARK["Batch Reports<br>(daily/weekly)<br>(Spark)"]
        ML["ML Models"]
...
flowchart TD
    subgraph PROD [" "]
        ...
        WRITERS["Writers<br>(Spark/Flink)"]
        READERS["Readers<br>(Trino/Spark)"]
        
        CAT_EXT["Catalog<br>(REST)"]
        REST["REST Catalog<br>(Tabular, Polaris, etc.)"]
        STORAGE["Object Storage<br>(S3 / GCS / ADLS)"]
