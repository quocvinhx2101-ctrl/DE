...
1. **Sự sụp đổ của Data Lake thuần tuý (Lake Swamps):**
   Năm 2015-2018, trào lưu "Cứ dump hết vào S3 rồi tính sau" đã tạo ra vô số "Đầm lầy dữ liệu". Data bị rác, schema không ai quản, query chậm rì rà vì Parquet files quá nhỏ. Đó là lý do **Lakehouse không phải là buzzword của Databricks, nó là cứu cánh thực sự**. Nếu bạn xây kho data mới năm 2025, **tuyệt đối không dùng Data Lake thuần nữa, phải dùng Lakehouse (Iceberg/Delta)**.

2. **Iceberg vs Delta Lake — Cuộc chiến ngã ngũ?**
   - **Delta Lake:** Tốt nhất nếu công ty bạn dùng Databricks từ A-Z. Rất mượt, performant tốt, bật auto-optimize là chạy. Nhưng nếu rời Databricks ra thì khá mệt.
   - **Apache Iceberg:** Đang chiến thắng ở vũ trụ **Open Ecosystem**. Snowflake sắm hẳn tài nguyên support Iceberg, AWS Athena native với Iceberg, Trino/Presto sinh ra là để cặp với Iceberg. Nếu công ty bạn dùng Multi-Cloud hoặc Multi-Engine, hãy chọn Iceberg. Hudi đang dần lùi về thị trường hẹp (streaming-heavy).
